...
=======
			if(findtext_char(message2recognize, "помощь"))
				say("Мои команды: Сделать объявление, Издать указ, Принять закон, Отменить закон, Удалить все законы, Объявить вне закона, Установить налоги, Призвать корону, Сменить Должность, Неважно")
>>>>>>> Stashed changes
...
=======
			if(findtext_char(message2recognize, "сменить должность"))
				if(notlord || nocrown)
					say("You are not my master!")
					playsound(src, 'sound/misc/machineno.ogg', 100, FALSE, -1)
					return
