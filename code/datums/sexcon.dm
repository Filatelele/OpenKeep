...
/datum/sex_controller/proc/begin_tailfap(obj/item/grabbing/G, mob/living/user)
	...
	if(user != owner)
		if(gender == MALE)
			owner.visible_message("<span class='[!owner.cmode ? "love" : "warning"]'>[tailfapping.grabbee] jerks [owner] with his tail!</span>")
		else
			owner.visible_message("<span class='[!owner.cmode ? "love" : "warning"]'>[tailfapping.grabbee] rubs [owner] with his tail!</span>")
	G.dependents += src
	START_PROCESSING(SSsex, user.sexcon)
	START_PROCESSING(SSsex, src)
